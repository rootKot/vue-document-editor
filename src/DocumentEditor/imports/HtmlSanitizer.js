...
export const HtmlSanitizer = new (function () {
    ...
    const _tagWhitelist = {
        'BODY': true, 'B': true, 'BLOCKQUOTE': true, 'BR': true, 'CENTER': true, 'DD': true, 'DIV': true, 'DL': true, 'DT': true, 'EM': true,
        'H1': true, 'H2': true, 'H3': true, 'H4': true, 'H5': true, 'H6': true, 'HR': true, 'I': true, 'LI': true, 'OL': true, 'P': true, 'PRE': true,
        'SMALL': true, 'SPAN': true, 'STRONG': true, 'SUB': true, 'SUP': true, 'UL': true, 'U': true
    };
    ...
    const _contentTagWhiteList = { 'A': true, 'DIV': true }; //tags that will be converted to DIVs

    const _attributeWhitelist = { 'align': true, 'style': true};

    const _cssWhitelist = { 'font-size': true, 'font-weight': true, 'text-align': true, 'text-decoration': true, 'white-space': true };

    const _schemaWhiteList = []; //which "protocols" are allowed in "href", "src" etc
        ...
        function makeSanitizedCopy(node) {
            ...
            if (node.nodeType == Node.TEXT_NODE) {
                ...
                if (_contentTagWhiteList[node.tagName])
                    newNode = doc.createElement('P'); //convert to DIV
                else
